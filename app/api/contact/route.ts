...
const generateEmailContent = (data: ContactMessageFields) => {
  ...
  const htmlData = Object.entries(data).reduce((str, [key, val]) => {
    return (
      str +
      `<h3 class="form-heading" align="left">${CONTACT_MESSAGE_FIELDS[key as keyof ContactMessageFields]}</h3><p class="form-answer" align="left">${val}</p>`
    );
  }, "");
...
//   console.log(data)
  ...
  if (!data || !data.name || !data.email || !data.subject || !data.message || !data.telephone) {
